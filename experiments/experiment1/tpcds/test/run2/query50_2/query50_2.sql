...
FROM store_sales AS ss,
  ...
  date_dim AS d2
WHERE
  d2.d_year = 1999 AND d2.d_moy = 9 AND ss.ss_ticket_number = sr.sr_ticket_number AND ss.ss_item_sk = sr.sr_item_sk AND ss.ss_sold_date_sk = d1.d_date_sk AND sr.sr_returned_date_sk = d2.d_date_sk AND ss.ss_customer_sk = sr.sr_customer_sk AND ss.ss_store_sk = s.s_store_sk
GROUP BY
  s.s_store_name,
  s.s_company_id,
  s.s_street_number,
