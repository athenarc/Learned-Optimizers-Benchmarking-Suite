...
FROM catalog_sales AS cs,
  ...
  item AS i
WHERE
  cs.cs_sold_date_sk = dd.d_date_sk AND cs.cs_item_sk = i.i_item_sk AND cs.cs_bill_cdemo_sk = cd1.cd_demo_sk AND cs.cs_bill_customer_sk = c.c_customer_sk AND cd1.cd_gender = 'M' AND cd1.cd_education_status = 'College' AND c.c_current_cdemo_sk = cd2.cd_demo_sk AND c.c_current_addr_sk = ca.ca_address_sk AND c.c_birth_month IN (9, 5, 12, 4, 1, 10) AND dd.d_year = 2001 AND ca.ca_state IN ('ND', 'WI', 'AL', 'NC', 'OK', 'MS', 'TN')
