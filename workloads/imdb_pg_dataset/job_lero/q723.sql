...
WHERE cct1.kind = 'cast'
    AND cct2.kind like '%ete%'
    AND chn.name is not NULL
    AND ( chn.name like '%man%'
    OR chn.name like '%Man%' )
    AND k.keyword in ( 'accidental-incest','phone-conversation','french-girl','student','hit-on-the-head-with-a-book','caller-hangs-up-when-phone-is-answered','reference-to-elvis-presley','hit-by-a-car','educational-video' )
    AND kt.kind = 'movie'
