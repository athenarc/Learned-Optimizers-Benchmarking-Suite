...
WHERE cct1.kind in ('complete+verified','crew','crew','complete+verified','crew','crew','cast')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[de]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'belief-in-hell'
    AND lt.link LIKE '% f%'
    AND mc.note IS NULL
    AND mi.info IN ('50','USA','USA:12 October 2004','English','Australia:M','Italian','English')
    AND t.production_year between 1998
    AND 2010
