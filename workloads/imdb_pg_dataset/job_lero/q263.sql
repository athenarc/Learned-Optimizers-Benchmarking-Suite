...
WHERE ci.note in ('(business & legal affairs)','(as Christy Carlson Romano)','(assistant producer)','(executive producer)','(producer)','(co-producer)')
    AND it1.info = 'LD disc format'
    AND it2.info = 'top 250 rank'
    AND k.keyword in ('islet','climbing')
