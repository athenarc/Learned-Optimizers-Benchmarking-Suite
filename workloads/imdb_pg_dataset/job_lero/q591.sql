...
WHERE cct1.kind in ('complete','complete+verified','cast','complete+verified','complete','complete+verified')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[in]'
    AND ( cn.name LIKE '% %'
    OR cn.name LIKE '% %' )
    AND ct.kind = 'distributors'
    AND k.keyword = 'supernatural'
    AND lt.link LIKE '% %'
    AND mc.note IS NULL
    AND mi.info IN ('Netherlands:16','Chile','Color')
    AND t.production_year between 1945
    AND 2008
