...
WHERE cct1.kind in ('cast','cast','crew','crew','complete','cast','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[mx]'
    AND ( cn.name LIKE '%i%'
    OR cn.name LIKE '%i%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'gambling'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('Drama','122','OFM:35 mm','PFM:16 mm','UK','Norwegian')
    AND t.production_year between 1925
    AND 2010
