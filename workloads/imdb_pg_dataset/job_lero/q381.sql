...
WHERE cct1.kind in ('cast','complete','complete+verified','complete+verified')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[jp]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'dc-comics'
    AND lt.link LIKE '%low%'
    AND mc.note IS NULL
    AND mi.info IN ('Warsaw, Mazowieckie, Poland','PFM:35 mm','English','USA:3 May 2012','English','Atlanta, Georgia, USA')
