...
WHERE cct1.kind = 'cast'
    ...
    AND it1.info = 'LD status of availablility'
    AND it2.info = 'votes'
    AND kt.kind in ('tv mini series','episode','video movie','video game','episode','movie','tv movie','tv mini series','movie')
    AND mc.note not like '% Germany)%'
    AND mi.info not IN ('Swedish','USA:X')
    AND mi_idx.info > '4.7'
