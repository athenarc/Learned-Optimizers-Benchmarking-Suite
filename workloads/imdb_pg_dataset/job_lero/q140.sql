...
WHERE cct1.kind = 'crew'
    AND cct2.kind like '%rifie%'
    AND chn.name is not NULL
    AND ( chn.name like '%%'
    OR chn.name like '%%' )
    AND it2.info = 'top 250 rank'
    AND k.keyword in ( 'murphy-bed','reference-to-george-romney','intruder-alert','chicken-soup','emperor','patent-medicine','crying-for-help','alexandria-egypt','imitation' )
    AND kt.kind = 'movie'
