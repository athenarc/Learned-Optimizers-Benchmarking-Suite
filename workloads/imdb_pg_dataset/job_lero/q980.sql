...
WHERE cct1.kind in ('complete','complete+verified','complete','complete+verified','complete','complete','cast','complete+verified','crew','cast')
    AND cct2.kind = 'complete+verified'
    AND it1.info = 'LD picture format'
    AND it2.info = 'rating'
    AND n.gender = 'm'
    AND t.production_year > 1955
    AND ( t.title like '%Fred%'
    OR t.title like '%è%'
    OR t.title like 'Saw%' )
