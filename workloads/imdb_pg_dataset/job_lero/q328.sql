...
WHERE cct1.kind in ('cast','complete+verified','cast','crew','complete','cast','crew')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[de]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'surprise-ending'
    AND lt.link LIKE '%m%'
    AND mc.note IS NULL
    AND mi.info IN ('Germany:75','Stage 2, The Lot - 1041 N. Formosa Avenue, West Hollywood, California, USA','USA:10 December 2001','Spain','Black
    AND White','English','Spain:5 March 2004','Japan')
    AND t.production_year between 1985
    AND 2004
