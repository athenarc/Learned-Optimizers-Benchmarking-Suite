...
WHERE cn.country_code != '[au]'
    AND ( cn.name LIKE '%i%'
    OR cn.name LIKE '%i%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'gunfight'
    AND lt.link LIKE '%b%'
    AND mc.note IS NULL
    AND mi.info IN ('MET:170 m','Black
    AND White','USA','Documentary','Argentina:10 March 2006','Color')
    AND t.production_year BETWEEN 1914
    AND 1978
