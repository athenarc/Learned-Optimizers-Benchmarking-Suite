...
WHERE cct1.kind in ('crew','cast','complete+verified','complete+verified','cast','cast','crew','complete','cast')
    AND cct2.kind = 'complete+verified'
    AND cn.country_code != '[co]'
    AND ( cn.name LIKE '%i%'
    OR cn.name LIKE '%i%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'amnesia'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('USA:15 February 2000','RAT:1.37 : 1','USA','USA','Color','RAT:1.66 : 1','Canada')
