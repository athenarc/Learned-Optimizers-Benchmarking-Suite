...
WHERE ci.note in ( '(adprom associate)','(insert unit coordinator)','(uncredited)','(stories)','(writer)','(executive producer)','(executive producer)' )
    ...
    AND n.name like '%Ang%'
    AND rt.role = 'actress'
    AND t.production_year between 2009
    AND 2009
