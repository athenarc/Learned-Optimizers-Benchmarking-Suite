...
WHERE cn.country_code != '[ar]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'three-word-title'
    AND lt.link LIKE '%d %'
    AND mc.note IS NULL
    AND mi.info IN ('90','None','OFM:Video','Horror','UK:5 July 2010','PFM:35 mm','Color','Color','UK:18 February 2013')
