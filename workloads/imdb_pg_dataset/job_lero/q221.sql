...
WHERE cct1.kind in ('crew','complete','complete','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[ba]'
    AND ( cn.name LIKE '%oduct%'
    OR cn.name LIKE '%oduct%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'suspect'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('Mono','Philippines:R-13','PCS:Betacam SP','Netherlands','30','English')
    AND t.production_year between 1977
    AND 2006
