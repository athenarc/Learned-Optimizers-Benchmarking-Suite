...
WHERE ci.note in ('(production assistant)','(writer)','(co-executive producer)','(coordinating producer)','(producer)','(production coordinator)','(business unit head)','(as BJ Karganilla)','(uncredited)','(co-executive producer)')
    AND it1.info = 'filming dates'
    AND it2.info = 'votes distribution'
    AND k.keyword in ('grown-man-cries','cancer-treatment','attack','failing-business','escape-from-prison')
