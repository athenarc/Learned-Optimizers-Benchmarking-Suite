...
WHERE ci.note in ('(story)','(voice)','(production coordinator)','(executive producer)','(segment producer)','(uncredited)')
    AND it1.info = 'alternate versions'
    AND it2.info = 'votes'
    AND k.keyword in ('playing','bedroom-community','work-break','animal-care')
