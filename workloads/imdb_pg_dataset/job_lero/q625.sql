...
WHERE cct1.kind = 'crew'
    AND cct2.kind like '%let%'
    AND chn.name is not NULL
    AND ( chn.name like '%man%'
    OR chn.name like '%Man%' )
    AND k.keyword in ( 'nymph','noah''s-ark','going-away-to-college','diaper-fetish','solving-crime','andy-warhol','caucasus-mountains','relocate','geofiction','sex-in-shower' )
