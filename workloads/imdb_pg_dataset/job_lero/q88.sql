...
WHERE cct1.kind in ('crew','complete','crew','complete','complete','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[ca]'
    AND ( cn.name LIKE '% %'
    OR cn.name LIKE '% %' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'photograph'
    AND lt.link LIKE '%fer%'
    AND mc.note IS NULL
    AND mi.info IN ('English','Spain','CAM:Panasonic AG-DVX100','USA','RAT:1.33 : 1','French','PFM:35 mm','UK','Greece')
