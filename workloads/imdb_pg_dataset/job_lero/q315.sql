...
WHERE cct1.kind in ('complete','crew','cast','cast','crew','crew','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[ca]'
    AND ( cn.name LIKE '% Pro%'
    OR cn.name LIKE '% Pro%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'manipulation'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('Music','France','Musical','PFM:35 mm','Color','USA','English')
    AND t.production_year between 2002
    AND 2005
