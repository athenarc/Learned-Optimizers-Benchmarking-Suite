...
WHERE cct1.kind in ('complete','crew','complete','cast','complete+verified','complete+verified','crew','complete+verified','crew','complete+verified')
    AND cct2.kind = 'complete+verified'
    AND cn.country_code != '[jp]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'stir-fry'
    AND lt.link LIKE '%n%'
    AND mc.note IS NULL
    AND mi.info IN ('English','Germany:14 February 2007','Italian','OFM:35 mm','Documentary')
    AND t.production_year between 1913
    AND 2012
