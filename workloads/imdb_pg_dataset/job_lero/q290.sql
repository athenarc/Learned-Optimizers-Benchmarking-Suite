...
WHERE cct1.kind in ('complete','complete+verified','crew','complete','cast','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[it]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'performer-name-in-title'
    AND lt.link LIKE '%spo%'
    AND mc.note IS NULL
    AND mi.info IN ('USA:8 October 2005','Australia','English')
    AND t.production_year between 1938
    AND 2000
