...
WHERE cct1.kind = 'complete'
    ...
    AND k.keyword in ('miracle','werner-erhard','undercover-marshal','missing-bus','severed-finger')
    AND kt.kind in ('tv mini series','tv series','movie','video movie','episode','tv mini series','video movie','video movie')
    AND mi.note like '%%'
    AND mi.info like '%a%'
