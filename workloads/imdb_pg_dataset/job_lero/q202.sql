...
WHERE cn.country_code != '[is]'
    AND ( cn.name LIKE '%i%'
    OR cn.name LIKE '%i%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'film-making'
    AND lt.link LIKE '%featur%'
    AND mc.note IS NULL
    AND mi.info IN ('USA:7 December 2011','Germany','Color','RAT:1.33 : 1','Animation','Romanian','English','English','Music','USA:TV-PG')
    AND t.production_year BETWEEN 1991
    AND 2013
