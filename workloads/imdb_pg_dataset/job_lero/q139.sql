...
WHERE cct1.kind = 'cast'
    AND cct2.kind like '%omp%'
    AND chn.name is not NULL
    AND ( chn.name like '%n %'
    OR chn.name like '%n %' )
    AND it2.info = 'rating'
    AND k.keyword in ( 'helicopter-crash','political-differences','divorced-family','amateur-magician','talking-like-chicken','undercover-hooker','bonsai' )
    AND kt.kind = 'episode'
