...
WHERE ci.note in ('(coordinating producer)','(segment producer)','(producer)','(as Rocsi Diaz)','(producer)','(co-executive producer)')
    AND it1.info = 'LD analog left'
    AND it2.info = 'top 250 rank'
    AND k.keyword in ('project-blue-book','ethnic-conflict','closed-military-base','room-service','nintendo-character','reference-to-marlboro-cigarettes','flash-bang','stinger-missile','hand-ripped-off','mother-died-at-childbirth')
