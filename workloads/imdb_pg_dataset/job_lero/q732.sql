...
WHERE ci.note in ('(production assistant)','(producer)','(production assistant: Boston)','(associate head writer)','(executive producer)','(associate segment producer)','(logger)','(written by)','(translator)')
    AND it1.info = 'gross'
    AND it2.info = 'top 250 rank'
    AND k.keyword in ('independent-music','house','booth','feisty-old-woman','professional-soccer-player')
