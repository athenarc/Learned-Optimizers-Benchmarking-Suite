...
WHERE cct1.kind in ('cast','crew','crew','crew','cast')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[il]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'hit-by-a-car'
    AND lt.link LIKE '%ence%'
    AND mc.note IS NULL
    AND mi.info IN ('USA','English','Documentary','Adult','60','USA','Dolby Digital','Color','English','CAM:Sony EX1')
