...
WHERE cct1.kind in ('cast','cast','cast','cast','complete+verified','cast','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[mx]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'alternative-comedy'
    AND lt.link LIKE '% %'
    AND mc.note IS NULL
    AND mi.info IN ('Color','12','Mono','USA:6 December 2008')
