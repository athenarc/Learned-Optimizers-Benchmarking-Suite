...
WHERE cct1.kind in ('cast','crew','complete+verified','complete','crew','crew','complete+verified','complete','cast')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[it]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'series-finale'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('Mono','2','Comedy','Color','Black
    AND White','Stage 7, Warner Brothers Burbank Studios - 4000 Warner Boulevard, Burbank, California, USA','India:U')
    AND t.production_year between 1962
    AND 2011
