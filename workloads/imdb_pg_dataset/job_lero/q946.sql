...
WHERE ci.note in ( '(writer)','(as Miki Manojlovic)','(voice)','(writer)','(production administration)','(accounting clerk)','(producer)','(supervising producer)','(archive footage)','(executive producer)' )
    ...
    AND n.name like '%Ang%'
    AND rt.role = 'actress'
    AND t.production_year between 1983
    AND 2005
