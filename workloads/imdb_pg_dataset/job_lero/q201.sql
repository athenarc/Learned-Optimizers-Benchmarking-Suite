...
WHERE cct1.kind = 'complete+verified'
    ...
    AND it1.info = 'plot'
    AND k.keyword in ('hideaway-bed','trespassing','reference-to-frédéric-auguste-bartholdi','barbados','birding','mispronounced-word')
    AND kt.kind in ('movie','movie','tv series','video game','video movie','tv series','tv series','video movie','episode')
    AND mi.note like '%n%'
    AND mi.info like '%%'
