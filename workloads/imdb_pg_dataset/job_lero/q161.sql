...
WHERE cct1.kind in ('complete+verified','complete','crew','crew','crew','crew','complete+verified','cast','complete','cast')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[jp]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'python'
    AND lt.link LIKE '%n%'
    AND mc.note IS NULL
    AND mi.info IN ('Short','Short','RAT:1.33 : 1','Color')
