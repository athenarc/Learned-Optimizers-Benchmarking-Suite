...
WHERE cct1.kind in ('cast','cast','cast','complete','complete','complete','complete','cast','cast')
    AND cct2.kind = 'complete+verified'
    AND cn.country_code != '[ca]'
    AND ( cn.name LIKE '%n%'
    OR cn.name LIKE '%n%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'mother-in-law'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('Black
    AND White','Black
    AND White','Short','History','RAT:1.33 : 1','USA','LAB:Technicolor')
    AND t.production_year between 1919
    AND 2009
