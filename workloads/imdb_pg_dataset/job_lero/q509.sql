...
WHERE cct1.kind = 'cast'
    AND cct2.kind like '%mple%'
    AND chn.name is not NULL
    AND ( chn.name like '%man%'
    OR chn.name like '%Man%' )
    AND k.keyword in ( 'man-walking-with-a-pram','nazca-plateau','listening-to-music-on-headphones','al-qaeda','death-by-stoning','donkey','american-civil-war','bunny-suit' )
