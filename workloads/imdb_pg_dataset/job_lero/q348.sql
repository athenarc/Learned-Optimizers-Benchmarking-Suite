...
WHERE ci.note in ('(co-producer)','(series composition)','(film commissioner)','(production assistant)','(teleplay)','(voice)','(director of photography)','(co-producer)','(executive producer)','(as Renée Elise Goldsberry)')
    AND it1.info = 'LD quality program'
    AND it2.info = 'votes'
    AND k.keyword in ('house-on-fire','swollen-tongue','blocked-chimney','fake-irishman','lashed-with-a-whip','compound')
