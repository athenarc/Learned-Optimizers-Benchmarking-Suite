...
WHERE cct1.kind in ('crew','cast','complete','crew')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[nz]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    ...
    AND lt.link LIKE '% f%'
    AND mc.note IS NULL
    AND mi.info IN ('6','Argentina:13','Short','Thriller','Color','German','PFM:35 mm','PCS:Spherical')
    AND t.production_year between 1971
    AND 2009
