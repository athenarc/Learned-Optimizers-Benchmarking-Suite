...
WHERE ci.note in ( '(voice: English version)','(voice)','(voice) (archive footage)','(technical director)','(production assistant)','(script editor)','(producer)','(uncredited)' )
    ...
    AND n.name like '%Ang%'
    AND rt.role = 'actress'
    AND t.production_year between 2010
    AND 2012
