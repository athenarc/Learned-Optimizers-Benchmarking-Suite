...
WHERE ci.note in ('(presenter)','(executive producer)','(script supervisor)','(producer)','(unit publicist)','(executive producer)')
    AND it1.info = 'LD number of chapter stops'
    AND it2.info = 'top 250 rank'
    AND k.keyword in ('country-folk-music','stolen-clothes','slow-motion-scene','grace','anti-war-demonstration','trying-to-conceive')
