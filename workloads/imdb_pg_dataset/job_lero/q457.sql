...
WHERE ci.note in ('(producer)','(producer)','(executive producer)','(post producer)','(production coordinator)','(producer)','(producer)','(writer)','(co-producer)','(story)')
    AND it1.info = 'printed media reviews'
    AND it2.info = 'votes'
    AND k.keyword in ('reference-to-lee-strasberg','spending-spree','anti-gambling','reference-to-syagrius','influenza','silver-spoon','jumping-through-a-window','m1-carbine-rifle')
