...
WHERE cct1.kind in ('cast','complete+verified','crew')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[hr]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'physician'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('USA:8 October 1983','USA')
    AND t.production_year between 2003
    AND 2012
