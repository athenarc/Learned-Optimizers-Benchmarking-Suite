...
WHERE ci.note in ( '(executive producer)','(uncredited)','(executive producer)','(production coordinator)','(line producer)','(production secretary)','(voice)' )
    ...
    AND n.name like '%Ang%'
    AND rt.role = 'actress'
    AND t.production_year between 1970
    AND 2013
