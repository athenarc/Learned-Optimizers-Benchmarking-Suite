...
WHERE ci.note in ('(location manager)','(montage)','(written by)','(voice)','(production assistant)','(executive producer)','(uncredited)')
    AND it1.info = 'LD release date'
    AND it2.info = 'top 250 rank'
    AND k.keyword in ('tomb','giving-blood','reference-to-scrooge','ship','caribbean','jewish-humor','nissan-navara','troop-transport','flotsam','fife')
