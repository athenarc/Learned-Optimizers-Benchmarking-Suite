...
WHERE cct1.kind in ('complete+verified','complete','complete+verified','crew','complete+verified','crew','crew','cast','crew')
    AND cct2.kind = 'complete+verified'
    AND cn.country_code != '[mx]'
    AND ( cn.name LIKE '%cti%'
    OR cn.name LIKE '%cti%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'brat'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('USA:9 January 2009','Black
    AND White','Mystery','PCS:Spherical','Taiwan','Mexico:90','Family','English','English','Color')
    AND t.production_year between 1994
    AND 2010
