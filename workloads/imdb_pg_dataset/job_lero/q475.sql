...
WHERE ci.note in ('(production coordinator)','(producer)','(producer)')
    AND it1.info = 'LD year'
    AND it2.info = 'votes distribution'
    AND k.keyword in ('turkey-as-food','rural','checkers-tournament','jaguar-xj','watchmaker','oscar-winner','robin-hood-plot')
