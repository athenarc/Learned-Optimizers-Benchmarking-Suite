...
WHERE cn.country_code != '[ca]'
    ...
    AND k.keyword = 'sitting-on-the-floor'
    AND lt.link LIKE '%nc%'
    AND mc.note IS NULL
    AND mi.info IN ('Black
    AND White','English','Drama','Philippines','USA:September 1896','Russian','PFM:35 mm','OFM:35 mm')
    AND t.production_year BETWEEN 1951
    AND 2010
