...
WHERE ci.note in ('(voice)','(associate producer)','(voice) (uncredited)','(producer)','(production assistant)','(creator)','(producer)')
    AND it1.info = 'LD master format'
    AND it2.info = 'votes distribution'
    AND k.keyword in ('undercover-policeman','arrow-in-back')
