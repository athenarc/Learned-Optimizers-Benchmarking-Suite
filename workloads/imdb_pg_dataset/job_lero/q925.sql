...
WHERE cct1.kind in ('complete+verified','cast','complete+verified','crew','complete+verified')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[us]'
    AND ( cn.name LIKE '%o%'
    OR cn.name LIKE '%o%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'long-take'
    AND lt.link LIKE '%ersion%'
    AND mc.note IS NULL
    AND mi.info IN ('USA:10 July 2012','OFM:35 mm','Black
    AND White','Comedy','MET:295 m','Color','Color','Silent')
    AND t.production_year between 1976
    AND 2007
