...
WHERE ci.note in ('(credit only)','(research team)','(producer)','(writer)','(supervising producer)','(supervising producer)','(voice)','(producer)','(producer)')
    AND it1.info = 'LD release date'
    AND it2.info = 'rating'
    AND k.keyword in ('convicted-felon','hidden-safe','world-war-two','secret-ingredient','town-council','alternative-history','long-hair','cruise-missile','cannon','morse-code')
