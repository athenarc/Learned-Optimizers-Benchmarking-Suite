...
WHERE cct1.kind in ('crew','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[nl]'
    AND ( cn.name LIKE '%%'
    OR cn.name LIKE '%%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'fire-axe'
    AND lt.link LIKE '%i%'
    AND mc.note IS NULL
    AND mi.info IN ('Israel','USA:26 September 2010','Animation','Color','USA')
    AND t.production_year between 1999
    AND 2012
