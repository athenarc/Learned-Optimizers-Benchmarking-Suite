...
WHERE cct1.kind in ('complete','crew','crew','crew','cast','complete')
    AND cct2.kind = 'complete'
    AND cn.country_code != '[jp]'
    AND ( cn.name LIKE '%uctio%'
    OR cn.name LIKE '%uctio%' )
    AND ct.kind = 'production companies'
    AND k.keyword = 'female-lead'
    AND lt.link LIKE '%%'
    AND mc.note IS NULL
    AND mi.info IN ('Silent','RAT:1.33 : 1','USA','UK','MET:300 m','Romance','English')
    AND t.production_year between 1981
    AND 2006
