...
WHERE ci.note in ('(supervising producer)','(producer)','(executive producer)','(creator)')
    AND it1.info = 'LD color information'
    AND it2.info = 'votes distribution'
    AND k.keyword in ('cellar','alamoheightssa','baby-bottle','grape','jack-knifed-truck','guessing-game','meta')
