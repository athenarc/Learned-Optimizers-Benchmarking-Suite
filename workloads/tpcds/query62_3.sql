...
FROM web_sales AS wsale,
  ...
  date_dim AS dd
WHERE
  dd.d_month_seq BETWEEN 1178 AND 1178 + 11 AND wsale.ws_ship_date_sk = dd.d_date_sk AND wsale.ws_warehouse_sk = w.w_warehouse_sk AND wsale.ws_ship_mode_sk = sm.sm_ship_mode_sk AND wsale.ws_web_site_sk = wsite.web_site_sk
GROUP BY
  SUBSTR(w.w_warehouse_name, 1, 20),
  sm.sm_type,
  wsite.web_name
