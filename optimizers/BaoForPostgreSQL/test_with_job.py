...
# Configuration
USE_BAO = True
PG_CONNECTION_STR = "dbname=tpcds user=suite_user host=train.darelab.athenarc.gr port=5468 password=71Vgfi4mUNPm"
FINAL_MODEL_DIR = "/data/hdd1/users/kmparmp/models/bao/tpc-ds/20250328_134134_bao_default_model"
...
def find_sql_files(directory):
    """Recursively find all .sql files in directories with less than 17 joins"""
    sql_files = []
    
    for root, dirs, files in os.walk(directory):
        # Extract join count from path if it exists
        join_match = re.search(r'(\d+)_joins', root)
        if join_match:
            join_count = int(join_match.group(1))
            if join_count >= 17:
                continue  # Skip directories with 17 or more joins
...
def run_query(sql, bao_select=False, bao_reward=False):
    ...
    while True:
        try:
            conn = psycopg2.connect(PG_CONNECTION_STR, options="-c statement_timeout=60000")
            cur = conn.cursor()
            cur.execute("SET statement_timeout = %s", ('60s',))  # timeout in milliseconds
