...
def recursively_process_queries(root_dir, frozen_data, db_string, save_base_path, skip_fastgres_processed=False):
    """
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        for query_file in filenames:
            if query_file.endswith('.sql'):
                if skip_fastgres_processed and os.path.exists(os.path.join(dirpath, "FASTgres")):
                    continue
        ...
        while count < NUM_EXECUTIONS:
            ...
            try:
                ...
                if NUM_EXECUTIONS > 1:
                    ...
                    plan_output_path = os.path.join(fastgres_dir, f"{queryId}_fastgres_plan.json")
                    result_file = os.path.join(fastgres_dir, f"{queryId}_fastgres_metrics.json")
