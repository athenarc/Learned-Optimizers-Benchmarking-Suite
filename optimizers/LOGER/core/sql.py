...
class _global:
    ...
    def plan_latency(self, sql, cache=True):
        ...
        self.__cur.execute(f"SET statement_timeout = 1800000;")
...
class Sql:
    ...
    def __init__(self, sql, feature_length=2, filename=None, device=torch.device('cpu'), table_space=None):
        ...
        if 'whereClause' in parse_result:
            where_clause = parse_result['whereClause']
            # If it's a compound expression (AND, OR, etc.)
            if 'BoolExpr' in where_clause:
                self.comparisons = [Comparison(arg, table_space=data_tables) for arg in where_clause['BoolExpr']['args']]
            # Otherwise, it's a single predicate
            else:
                self.comparisons = [Comparison(where_clause, table_space=data_tables)]
    ...
    def parse_join(self):
        ...
        for alias in self.aliases:
            ...
            self.table_feature_others[alias] = torch.tensor(self.table_feature_others[alias], dtype=torch.float32).unsqueeze(0).detach()
