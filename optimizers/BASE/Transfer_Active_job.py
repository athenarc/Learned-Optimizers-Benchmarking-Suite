...
def save_train_data():
    ...
    for idx, sql_query in enumerate(train_files):
        sql = sqlparse.format(sql_query, reindent=False, keyword_case="upper")
        ...
        try:
            ...
            print('problem plan: {}'.format(sql_query))
            problem_plan.append(sql_query)
    ...
    for idx, sql_query in enumerate(test_files):
        sql = sqlparse.format(sql_query, reindent=False, keyword_case="upper")
        SQL_encode = travesal_SQL(sql)
        query_encode = SQL_encode
        join_table = get_join_table(query_encode[2])
...
def test(model, job=False):
    ...
    if job:
        ...
        for query in test_files:
            sql = sqlparse.format(query, reindent=False, keyword_case="upper")
            execute(sql)
...
def interactive_training(model):
    ...
    def test(model, epoch):
        ...
        reward_log.append(sum(all) / len(test_files))
...
def get_entroy(model):
    ...
    for epoch in range(-1, 173, 1):
        ...
        assert len(entropy_log) == len(test_files)
...
def Inverse_RL_one_sql(policy_net, discrim_net):
    ...
    def train_policy(sql, policy_net, policy_optim, discrim_net, policy_update_num):
        ...
        for step in range(policy_update_num):
            batch_loss = []
            for idx, query in enumerate(test_files):
                sql = sqlparse.format(query, reindent=False, keyword_case="upper")
                SQL_encode = travesal_SQL(sql)
    ...
    def test(model, epoch):
        ...
        for query in test_files:
            ...
            if terminate_plan == -1:
                ...
                all.append(float(terminate_plan['Execution Time']))
        all = np.array(all)
        PG_late = np.array([float(terminate_plan['Plan']['Total Cost']) for query in test_files])
        cur = pow(np.prod(all / PG_late), 1 / len(all))
        print(cur)
        reward_log.append((sum(all) / len(test_files), cur))
        ...
        state = {'model': model.state_dict(), 'result': sum(all) / len(test_files)}
        torch.save(state, checkpoint_path)
    ...
    for iter in range(1, max_episode):
        ...
        chosen_queries = [test_files[i] for i in chosen]
...
def save_train_data_():
    ...
    if not os.path.exists('./train_plan/Imitation_Learning/x_expert_train.npy'):
        ...
        problem_plan = []
        # problem = np.load('./train_plan/Neo_baseline/problem_plan.npy').tolist()
        for idx, query in enumerate(train_files):
            sql = sqlparse.format(query, reindent=False, keyword_case="upper")
            SQL_encode = travesal_SQL(sql)
            query_encode = SQL_encode
            join_table = get_join_table(query_encode[2])
...
if __name__ == '__main__':
    ...
    new_cp = json.load(open('reward_log_NEW.json', encoding='utf-8'))
    # chekpoint    
    min_idx = 98
    dir_list = os.listdir('./Models/Model_checkpoint')
    new_list = [int(i[4:].split('.')[0]) for i in dir_list]
    new_list.sort()
    checkpoint_path = './Models/Model_checkpoint/now_{}.pth'.format(new_list[min_idx])
    # checkpoint_path = 'Models/now.pth'
    cp = torch.load(checkpoint_path, map_location='cpu')

    # Create mapping between old and new keys
    key_mapping = {
        '0.hidden1.weight': 'encode.hidden1.weight',
        '0.hidden1.bias': 'encode.hidden1.bias',
        '0.hidden2.weight': 'encode.hidden2.weight',
        '0.hidden2.bias': 'encode.hidden2.bias',
        '0.hidden3.weight': 'encode.hidden3.weight',
        '0.hidden3.bias': 'encode.hidden3.bias',
        '1.weights.weight': 'BTC1.weights.weight',
        '4.weights.weight': 'BTC2.weights.weight',
        '7.weights.weight': 'BTC3.weights.weight',
        '11.hidden1.weight': 'hidden1.weight',
        '11.hidden1.bias': 'Pi.hidden1.bias',
        '11.hidden2.weight': 'Pi.hidden2.weight',
        '11.hidden2.bias': 'Pi.hidden2.bias',
        '11.hidden3.weight': 'Pi.hidden3.weight',
        '11.hidden3.bias': 'Pi.hidden3.bias',
        '11.hidden4.weight': 'Pi.hidden4.weight',
        '11.hidden4.bias': 'Pi.hidden4.bias'
    }

    # Create new state_dict with renamed keys
    new_state_dict = {key_mapping.get(k, k): v for k, v in cp['model'].items()}
    ...
    # discrim_net.load_state_dict(new_state_dict)
