...
flags.DEFINE_integer('max_epochs', 100, 'Maximum number of training epochs (value iterations).')
flags.DEFINE_integer('min_epochs_for_loss_stop', 50, 'Minimum epochs before early stopping can occur.')
flags.DEFINE_float('loss_threshold', 0.01, 'Validation loss threshold for early stopping.')

flags.DEFINE_string('checkpoint_dir_base', None, 'Base directory to save all checkpoints and the final model.')
flags.DEFINE_integer('checkpoint_epoch_interval', 10, 'Save a checkpoint every N epochs.')
flags.DEFINE_integer('checkpoint_query_interval', 250, 'Save a checkpoint every N queries executed.')
flags.DEFINE_float('loss_improvement_threshold', 0.001, 'Save checkpoint on loss improvement.')
flags.DEFINE_string('resume_checkpoint_path', None, 'Path to a specific checkpoint to resume training from.')
...
class BalsaAgent(object):
    ...
    def __init__(self, params):
        ...
        # We'll just add the new checkpoint directories
        if params.checkpoint_dir_base:
            self.checkpoint_dir_base = Path(params.checkpoint_dir_base)
            ...
            # This is required for the new logic
            raise ValueError("--checkpoint_dir_base must be specified.")
        
        if p.resume_checkpoint_path:
            self.resume_checkpoint_path = p.resume_checkpoint_path
            print(f"Resuming training from {p.resume_checkpoint_path}")
            self._ResumeFromCheckpoint(p.resume_checkpoint_path)
    ...
    def _ResumeFromCheckpoint(self, checkpoint_path):
        ...
        queries_str = [p for p in parts if p.startswith("queries-")][0]

        self.start_epoch = int(epoch_str.split("-")[1])
        self.curr_value_iter = self.start_epoch
        self.num_query_execs = int(queries_str.split("-")[1])

        print(f"Resuming from epoch {self.start_epoch}, queries {self.num_query_execs}")
        ...
        # Re-init dataset + model like evaluate_checkpoint
        train_ds, _, _, _ = self._MakeDatasetAndLoader(log=False)
        plans_dataset = train_ds.dataset if isinstance(train_ds, torch.utils.data.Subset) else train_ds
        model_pl = self._MakeModel(plans_dataset)

        state = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)
        
        # --- START: ROBUST LOADING FIX ---
        raw_state_dict = state['state_dict'] if 'state_dict' in state else state

        # Check if keys are in the old format (missing the 'model.' prefix).
        # We only need to check one key to know the format.
        first_key = next(iter(raw_state_dict))
        if not first_key.startswith('model.'):
            print("Detected old checkpoint format. Adding 'model.' prefix to keys...")
            ...
            final_state_dict = raw_state_dict
                
        model_pl.load_state_dict(final_state_dict)
        self.model = model_pl.model

        if 'optimizer_state_dict' in state:
            self.prev_optimizer_state_dict = state['optimizer_state_dict']

        print(f"Successfully loaded model + optimizer state from {checkpoint_path}")
    ...
    def SaveAgent(self, model, target_dir):
        """Saves the agent's model to a specific target directory."""
        # The original function saved to multiple places. We simplify it.
        # It saves a PyTorch Lightning model object.
        
        # Create a unique name for the artifact
        prefix = f"epoch-{self.curr_value_iter:03d}_queries-{self.num_query_execs}_loss-{self.best_val_loss:.6f}"
        
        # The main model checkpoint
        ckpt_path = os.path.join(target_dir, f"{prefix}_checkpoint.pt")
        torch.save(model.state_dict(), ckpt_path)

        # Metadata about the checkpoint
        metadata_path = os.path.join(target_dir, f"{prefix}_metadata.txt")
        SaveText(f'value_iter,{self.curr_value_iter}', metadata_path)

        print(f"Saved checkpoint to: {target_dir}")
    ...
    def Run(self):
        ...
        if not p.resume_checkpoint_path:
            self.curr_value_iter = 0
            self.overall_best_train_latency = np.inf
            self.overall_best_test_latency = np.inf
            self.overall_best_test_swa_latency = np.inf
        ...
        # --- Independent Progress Trackers ---
        best_actual_loss = float('inf')
        next_epoch_checkpoint_target = p.checkpoint_epoch_interval
        next_query_checkpoint_target = p.checkpoint_query_interval

        # ===============================================
        # MAIN EPOCH (VALUE ITERATION) LOOP
        # ===============================================
        while self.curr_value_iter < p.max_epochs:
            self.curr_iter_skipped_queries = 0
            
            # --- 1. Train the model for one epoch ---
            # The original RunOneIter() is now the body of our loop
            self.timer.Start('train')
            train_ds, train_loader, _, val_loader = self._MakeDatasetAndLoader(log=True)
            plans_dataset = train_ds.dataset if isinstance(train_ds, torch.utils.data.Subset) else train_ds
            
            model = self._MakeModel(plans_dataset)
            model.SetLoggingPrefix(f'train/iter-{self.curr_value_iter}-')
            
            trainer = self._MakeTrainer(train_loader)
            trainer.fit(model, train_loader, val_loader)
            self.model = model.model
            
            # This is our actual validation loss for this epoch
            current_best_val_loss = trainer.early_stop_callback.best_score.item()
            self.best_val_loss = current_best_val_loss # Store for filenames
            
            self._LoadBestCheckpointForEval(model, trainer)
            self.timer.Stop('train')

            # --- 2. Plan and Execute Queries to Gather New Experience ---
            ...
            # END OF EPOCH LOGIC
            # ===============================================
            print(f"\nEPOCH {self.curr_value_iter} COMPLETE. Queries Executed this Epoch: {len(to_execute)}. Best Val Loss: {current_best_val_loss:.8f}")

            resume_epoch_dir = Path(self.checkpoint_dir_base).parent / "resume_epoch_checkpoints"
            resume_epoch_dir.mkdir(parents=True, exist_ok=True)
            self.SaveAgent(model, resume_epoch_dir)
            
            # --- Checkpointing ---
            if self.curr_value_iter >= next_epoch_checkpoint_target:
                self.SaveAgent(model, self.epoch_checkpoint_dir)
                next_epoch_checkpoint_target += p.checkpoint_epoch_interval

            if self.num_query_execs >= next_query_checkpoint_target:
                self.SaveAgent(model, self.query_checkpoint_dir)
                next_query_checkpoint_target += p.checkpoint_query_interval
            
            if (best_actual_loss - current_best_val_loss) > p.loss_improvement_threshold:
                self.SaveAgent(model, self.loss_checkpoint_dir)
                best_actual_loss = current_best_val_loss
            
            # --- Stopping Criterion ---
            condition1_met = (current_best_val_loss < p.loss_threshold) and (self.curr_value_iter > p.min_epochs_for_loss_stop)
            
            if condition1_met:
                print(f"\nSTOPPING: Validation Loss ({current_best_val_loss:.8f}) is below threshold ({p.loss_threshold}).")
                self.SaveAgent(model, self.final_model_dir)
                break
            
            # --- Test Set Evaluation (original logic) ---
            self.EvaluateTestSet(model, planner)
...
def Main(argv):
    ...
    p.max_epochs = FLAGS.max_epochs
    p.min_epochs_for_loss_stop = FLAGS.min_epochs_for_loss_stop
    p.loss_threshold = FLAGS.loss_threshold
    p.checkpoint_dir_base = FLAGS.checkpoint_dir_base
    p.checkpoint_epoch_interval = FLAGS.checkpoint_epoch_interval
    p.checkpoint_query_interval = FLAGS.checkpoint_query_interval
    p.loss_improvement_threshold = FLAGS.loss_improvement_threshold    
    ...
    if FLAGS.resume_checkpoint_path:
        print(f"Resuming from checkpoint: {FLAGS.resume_checkpoint_path}")
        p.resume_checkpoint_path = FLAGS.resume_checkpoint_path
