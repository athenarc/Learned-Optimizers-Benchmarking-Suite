...
class BalsaAgent(object):
    ...
    def RunOneIter(self):
        ...
        # Timer for the entire iteration
        ...
        # Logging.
        if not has_timeouts:
            self.overall_best_train_latency = min(self.overall_best_train_latency, iter_total_latency / 1e3)
            to_log = [
                ('latency/workload', iter_total_latency / 1e3, self.curr_value_iter),
                ('latency/workload_best', self.overall_best_train_latency, self.curr_value_iter),
                ('num_query_execs', self.num_query_execs, self.curr_value_iter),
                ('num_queries_with_eps_random', planner.num_queries_with_random, self.curr_value_iter),
                ('curr_iter_skipped_queries', self.curr_iter_skipped_queries, self.curr_value_iter),
                ('curr_value_iter', self.curr_value_iter, self.curr_value_iter),
                ('lr', model.learning_rate, self.curr_value_iter),
            ]
            if p.reduce_lr_within_val_iter:
                to_log.append(('iter_final_lr', model.latest_per_iter_lr, self.curr_value_iter))
            self.LogScalars(to_log)

        self.SaveBestPlans()

        if (self.curr_value_iter + 1) % 5 == 0:
            start_time_save_agent = time.time()
            self.SaveAgent(model, iter_total_latency, curr_value_iter=self.curr_value_iter)
            save_agent_time = time.time() - start_time_save_agent
        else:
            save_agent_time = 0.0

        # Run and log test queries.
        start_time_evaluate_test = time.time()
        self.EvaluateTestSet(model, planner)
        evaluate_test_time = time.time() - start_time_evaluate_test
        ...
        if p.track_model_moving_averages:
            ...
            if self.curr_value_iter >= 0:
                ...
                self.UpdateMovingAverage(model, moving_average='ema', ema_decay=p.ema_decay)
                ema_time = time.time() - start_time_ema

                if (self.curr_value_iter + 1) % 5 == 0:
                    start_time_ema_test = time.time()
        ...
        # Log the time taken by each step
        total_time_iter = time.time() - start_time_iter
        time_log = [
            ('time/train', train_time, self.curr_value_iter),
            ('time/replay_reset', replay_reset_time, self.curr_value_iter),
            ('time/plan_execute', plan_execute_time, self.curr_value_iter),
            ('time/feedback', feedback_time, self.curr_value_iter),
            ('time/save_agent', save_agent_time, self.curr_value_iter),
            ('time/evaluate_test', evaluate_test_time, self.curr_value_iter),
            ('time/ema', ema_time, self.curr_value_iter),
            ('time/ema_test', ema_test_time, self.curr_value_iter),
            ('time/swa', swa_time, self.curr_value_iter),
            ('time/swa_test', swa_test_time, self.curr_value_iter),
            ('time/total_iter', total_time_iter, self.curr_value_iter),
        ]
        
        import json

        time_log_dict = [{"event": event, "time": time, "iteration": iteration} for event, time, iteration in time_log]

        with open("time_log.json", "w") as file:
            json.dump(time_log_dict, file, indent=4)
