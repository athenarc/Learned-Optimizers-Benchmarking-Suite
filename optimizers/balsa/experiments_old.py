...
@balsa.params_registry.Register
class Neo_TPC_H_EvaluationBase(NeoImplRand52):
    def Params(self):
        p = super().Params()
        p.db = 'tpch'
        p.query_dir = 'queries/tpch'
...
@balsa.params_registry.Register
class Neo_TPC_DS_EvaluationBase(NeoImplRand52):
    def Params(self):
        p = super().Params()
        p.db = 'tpcds'
        p.query_dir = 'queries/tpcds'
        p.query_glob = ['*.sql']
        p.test_query_glob = ['query18_*.sql'] + ['query27_*.sql'] + ['query52_*.sql'] + ['query82_*.sql'] + ['query98_*.sql']
        p.epochs = 50
        p.val_iters = 50
        p.sim_checkpoint = None        
        p.init_experience = 'data/initial_policy_data__tpcds.pkl'
