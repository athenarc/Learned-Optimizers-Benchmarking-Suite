...
class DataBaseEnv_FOOP_QueryEncoding(DataBaseEnv_FOOP, DataBaseEnv_QueryEncoding):
    ...
    def __init__(self, env_config, is_join_graph_encoding=False):
        ...
        self.observation_space = Dict({
            'real_obs': Box(low = 0, high = 1, shape = real_obs_shape, dtype = np.int32),
            'action_mask': Box(low = 0, high = 1, shape = (len(self.actions), ), dtype = np.int32),
        })
    ...
    def get_obs(self):
        ...
        real_obs = np.concatenate(real_obs).astype(np.int32)
        return {
            'real_obs': real_obs.tolist(),
            'action_mask': self.valid_actions().astype(np.int32).tolist()
        }
