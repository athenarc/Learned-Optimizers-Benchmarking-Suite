...
from utils.db_utils import get_cost_plan, explain_analyze_plan
import time
import sys
...
class DataBaseWithSelectQueryStoreEnv(gym.Env):
    ...
    def reward(self):
        def _get_cost(_retry=0):
            try:
                ...
                # Get the cost of the current plan
    ...
    def explain_analyze(self):
        """
        Get the explain analyze of the current plan
        """
        if self.plan:
            return explain_analyze_plan(self.plan, self.conn, self.db)
        else:
            raise Exception("No plan available to explain")
