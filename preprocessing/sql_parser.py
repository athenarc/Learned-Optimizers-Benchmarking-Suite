...
def extract_where_part(parsed, split_parentheses=False):
    """Extract predicates from the WHERE clause."""
    predicates = []
    for item in parsed.tokens:
        if isinstance(item, Where):
            where_tokens = list(item.flatten())
            predicates.extend(process_where_tokens(where_tokens, split_parentheses))
    return predicates

def process_where_tokens(tokens, split_parentheses=False):
    ...
    def handle_parenthesis():
        ...
        if split_parentheses:
            # When splitting, we treat the content inside parentheses as separate predicates
            ...
            # Original behavior - treat as one big predicate
            parenthesis_start = i
            parenthesis_depth = 0
...
def parse_sql(query, split_parentheses=False, db_schema = None):
    parsed = sqlparse.parse(query)[0]
    columns = list(extract_select_part(parsed))
    tables = list(extract_from_part(parsed))
    predicates = extract_where_part(parsed, split_parentheses)
