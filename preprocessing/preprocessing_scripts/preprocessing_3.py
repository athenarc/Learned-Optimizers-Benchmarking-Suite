...
def process_queries(query_directory, train_output_dir: Path, test_output_dir: Path, dbname=None, port=5469): 
    ...
    try:
        ...
        while run < RUNS:
            for count, (query, file_location, query_name) in enumerate(queries):
                ...
                process_single_query(run, query, query_name, test_output_dir, dbname, port)
...
def process_single_query(runID, query, query_name, output_dir: Path, dbname=None, port=5469):
    ...
    try:
        ...
        # Save the QEP data to a JSON file
        if RUNS > 1:
            output_dir_run = output_dir / f"run{runID + 1}"
        else:
            output_dir_run = output_dir
        qep_output_path = output_dir_run / queryId / f"classic_qep.json"
        ...
        if RUNS > 1:
            ...
            query_output_path = output_dir / f"{queryId}" / f"{query_name}"
...
if __name__ == "__main__":
    ...
    TRAIN_OUTPUT_DIR = Path("../../experiments/experiment3/train/")
    TEST_OUTPUT_DIR = Path("../../experiments/experiment3/test/")
    TRAIN_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    TEST_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    process_queries(QUERY_DIRECTORY, TRAIN_OUTPUT_DIR, TEST_OUTPUT_DIR, dbname='imdbload', port=5469)
